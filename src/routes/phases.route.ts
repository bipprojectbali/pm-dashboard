...
const PHASE_STATUS_VALUES = ['PLANNING', 'ACTIVE', 'COMPLETED'] as const
...
export function phasesRoutes() {
  return new Elysia()
    .get('/api/phases', async ({ request, set }) => {
    ...
    .get('/api/projects/:id/phases', async ({ request, params, set }) => {
      const auth = await requireAuth(request)
      if (!auth) {
        set.status = 401
        return { error: 'Unauthorized' }
    ...
    .post('/api/projects/:id/phases', async ({ request, params, set }) => {
      const auth = await requireAuth(request)
      if (!auth) {
        set.status = 401
        return { error: 'Unauthorized' }
    ...
    .patch('/api/phases/:id', async ({ request, params, set }) => {
      const auth = await requireAuth(request)
      if (!auth) {
        set.status = 401
        return { error: 'Unauthorized' }
    ...
    .delete('/api/phases/:id', async ({ request, params, set }) => {
      const auth = await requireAuth(request)
      if (!auth) {
        set.status = 401
        return { error: 'Unauthorized' }
