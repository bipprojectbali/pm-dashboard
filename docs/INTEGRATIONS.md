...
- **Auth**: `Authorization: Bearer pmt_…` (token-only, tidak terima session). Auto-scoped ke project token; agent tak pernah kirim `projectId`. READ → GET; WRITE → + POST/PATCH/DELETE. IDEA read-only. Cross-project → 404 no-leak. Input invalid (enum/JSON malformed) → 400, bukan 500.
- **Endpoint**: list (paginated: `page`/`limit`, response `{count,page,limit,total,totalPages,tasks}`), **stats** (`GET /api/agent/tasks/stats` — groupBy status/kind/priority), get (rich: comments/evidence/history/checklist-with-ids), **project metadata** (`GET /api/agent/project`), create/update(+transition)/comment/checklist di `/api/agent/*`. Lihat `@docs/API.md` § Agent REST API. Route modular `src/routes/agent/*` (barrel `index.ts`), helper `src/lib/agent-auth.ts` + `src/lib/task-enums.ts`.
- **CLI wrapper**: `scripts/agent-cli.ts` (`bun run agent <cmd>`) baca `.env.agent` (`PM_API_TOKEN` + `PM_GUIDE_URL`; base API di-derive dari guide URL) → bungkus surface ini: `tasks`, `stats`, `get <id>`, `project`, `guide`, `create`, `comment`. Menjadikan `.env.agent` berguna (sebelumnya orphan). Dokumentasi var di `.env.example`.
