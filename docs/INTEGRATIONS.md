...
- **Endpoint**: list/get/create/update(+transition)/comment/checklist di `/api/agent/*`. Lihat `@docs/API.md` § Agent REST API. Route `src/routes/agent.route.ts` (barrel yang meng-`use` submodul `src/routes/agent/{guide,tasks,checklist}.route.ts` + `shared.ts`), helper `src/lib/agent-auth.ts`.
