...
export function useTasksPanelState({ projectId, onProjectChange, canWriteOverride }: Opts) {
  ...
  const changeProject = (id: string | null) => { setTagFilter(null); setPhaseFilter(null); onProjectChange?.(id) }
  const openTask = (id: string) => setDrawerTaskId(id)
  const closeTask = () => { setDrawerTaskId(null); qc.invalidateQueries({ queryKey: ['tasks'] }) }
  const clearSelection = () => setSelectedIds(new Set())

  const { create, bulkCreate, deleteOne, deleteBulk } = useTaskMutations({
    onCreateSuccess: () => setCreateOpen(false),
    onDeleteOneSuccess: (id) => setSelectedIds((prev) => {
  ...
  const canDeleteTask = useCallback(
    (t: TaskListItem) => isAdmin || (currentUserId != null && t.reporter.id === currentUserId) || leadProjectIds.has(t.projectId),
    [isAdmin, currentUserId, leadProjectIds],
  )

  const rawTasks = tasksQ.data?.tasks ?? []
  const total = tasksQ.data?.total ?? 0
  const tasks = useMemo(() => filterAndSortTasks(rawTasks, quickFilter, dueDateRange, sortBy, sortDir), [rawTasks, quickFilter, dueDateRange, sortBy, sortDir])
  ...
  const deletableSelected = useMemo(
    () => Array.from(selectedIds).filter((id) => { const t = taskById.get(id); return t ? canDeleteTask(t) : false }),
  ...
  const toggleAllSelection = () => setSelectedIds((prev) => {
  ...
  const confirmDeleteOne = (t: TaskListItem) => modals.open({
