...
export function adminUsersRoutes() {
  return new Elysia()
    ...
    .get('/api/admin/users', async ({ request, query, set }) => {
      ...
      // Optional server-side filters. Backward-compatible: without `limit`,
      // returns the full roster (legacy shape { users }) so existing callers
      // (OverviewPanel, AuditLogsPanel) keep working. With `limit`, returns
      // a paginated shape { users, total, limit, offset }.
      const search = typeof query.search === 'string' ? query.search.trim() : ''
      const roleFilter = typeof query.role === 'string' ? query.role : ''
      if (roleFilter && !['USER', 'QC', 'ADMIN', 'SUPER_ADMIN'].includes(roleFilter)) {
        set.status = 400
      ...
      const hasPagination = query.limit !== undefined
      if (!hasPagination) {
      ...
      const [users, total] = await Promise.all([
        prisma.user.findMany({ where, select, orderBy: { createdAt: 'asc' }, take: limit, skip: offset }),
