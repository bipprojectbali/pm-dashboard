...
- `POST /api/admin/chat/stream` — SSE stream. Body: `{ messages: ChatMessage[], systemContext?: string|null }`. Events: `phase` (`{ label }` — teks status human-readable yang dirender FE selama loading; loop ke Claude juga mengikutkan `phase`+`iter`. Label: iter 0 `"Menganalisis pertanyaan..."`, iter 1+ `"Menyusun jawaban..."`, dan `"Mencari data..."` diemit saat tool mulai dieksekusi pada iterasi mana pun), `system` (kalau cachedContext null), `docs` (count relevan), `sources` (`[{ ref, type, entityId, title }]`), `tool_use` (`{ id, name, input }`), `tool_result` (`{ id, name, ok, result }`), `token`, `done` (full text + sources + toolCalls trace), `error`. AI dapat memanggil 5 tool read-only via Anthropic tool_use loop (max 5 iterasi); lihat `@docs/CHAT-AI.md` § Tool-calling layer.
...
- `GET /api/projects/:id` — full detail (members, milestones, extensions, recent tasks) + `myRole`. `owner` and `members[].user` both include `blocked` — the row itself is **not** filtered (blocking a user never removes their `ProjectMember`), only the flag is exposed so callers can decide what to do with it. The frontend uses this to exclude blocked users from active-member pickers (Projects "Anggota" avatar/filter strip, task assignee selects in `CreateTaskModal`/`TaskDetailSidebar`) while still showing them — with a "Blocked" badge — in administrative member lists (`MembersSection` Team tab, project Overview tab's Team card) so an admin can still see and remove them. Same `blocked` field is on `GET /api/projects` list rows too (shared `PROJECT_INCLUDE`).
...
- `GET /api/tasks` — list with filters (`projectId`, `status`, `kind`, `assigneeId`, `tagId`, `priority`, `search`, `mine`, `unassigned`, `blocked`, `openOnly`). `kind` ∈ `TASK | BUG | QC | TICKET | IDEA` (400 on invalid). Response enriches each task with `actualHours`, `progressPercent`, `tags`, counts for blockedBy/blocks/checklist. **Sort (server-side):** `sort` ∈ `dueAt | priority | title | createdAt | updatedAt | estimateHours` + `dir` ∈ `asc | desc` (400 on invalid). Nullable sorts (`dueAt`, `estimateHours`) place NULLs last; a stable `id:asc` tiebreak keeps pagination deterministic. No `sort` → default status-asc order (OPEN…CLOSED last, so a truncating `limit` drops CLOSED before open rows — locked by `tests/integration/tasks-list-order.test.ts`). **Due-date filters (server-side, compose into one `dueAt` clause):** `overdueOnly=1` (`dueAt < now` AND not CLOSED), `noDue=1` (no due date, wins over range), `dueFrom`/`dueTo` (ISO; inclusive range, `dueTo` snapped to end-of-day) — `overdueOnly` + `dueFrom`/`dueTo` combine. All sort/filter/pagination is server-side so ordering and counts stay correct across pages (the `/pm` Task board issues these instead of re-sorting the current page client-side). **Opt-in `staleDays`** (numeric, clamp 1–30): when sent, keeps only open tasks not updated in the last N days (forces open-only unless a status is given) — used by the Tiket/Pengembangan board "Stale" quick-filter; absent → no effect (menu Task/admin surfaces unaffected). Visibility scoping (`src/routes/tasks/list.helpers.ts` `taskVisibilityWhere`): system admins see all; others see tasks in a project they're a member of OR whose project `visibility` is `INTERNAL`/`PUBLIC`. Helpers: `src/routes/tasks/list.helpers.ts` (`parseTaskSort`, `buildTaskListOrderBy`, `buildDueAtCondition`, `taskVisibilityWhere`).
- `GET /api/tasks/kind-board-stats?kind=TICKET|IDEA&staleDays=` — stat-card counts for the `/pm` Tiket/Pengembangan boards: `{ total, overdue, unassigned, blocked, stale, staleDays }` over **open** tasks of that one `kind` (400 if kind ∉ TICKET/IDEA). Counted in-DB (un-capped) and **visibility-scoped** to the caller (same `taskVisibilityWhere` clause), so cards stay correct regardless of the table's page size. User-facing (any logged-in user); distinct from the admin-only `/api/admin/overview/triage` (which counts all tasks and excludes IDEA). Lib: `src/lib/kind-board-stats.ts` (`computeKindBoardStats`).
- `GET /api/tasks/dashboard-stats?projectId=` — Total/Open/Closed/Overdue for the Tasks panel dashboard overlay (`TaskDashboardOverlay`, both the `/pm` global Task board and a project's Tasks tab). `projectId` optional (omit for the visibility-scoped global count). Counted in-DB (un-capped), **includes every `kind`** (does not exclude IDEA — matches the overlay's pre-existing behavior). 403 if `projectId` given and the caller can't read that project. Fixes a bug where the overlay derived these cards from `tasks.length` over the `/api/tasks` **200-row cap**, silently under-counting past ~200 tasks (same bug class as the Admin Analytics/Task Triage cards). Lib: `src/lib/task-dashboard-stats.ts` (`computeTaskDashboardStats`).
- `GET /api/tasks/dashboard-charts?projectId=&trendDays=` — the three charts on the same `TaskDashboardOverlay` (Throughput, Status breakdown, Top assignees). Returns `{ throughput: {date,created,closed}[], statusBreakdown: Record<status,count>, topAssignees: {id,name,count}[] }`. `trendDays` clamps 1–90, default 14 (mirrors `computeAnalytics`'s trend window). `topAssignees` is the top 8 by open-task count (status ≠ CLOSED), desc. Same scoping/gating as `dashboard-stats` (`projectId` optional, 403 if inaccessible, **includes every `kind`** to stay consistent with the stat cards it sits under). Counted in-DB (un-capped) — fixes the same 200-row-cap bug for the charts (they previously derived from a client-side `useMemo` over a capped `/api/tasks?pageSize=500` fetch that the server still hard-capped at 200). Lib: `src/lib/task-dashboard-charts.ts` (`computeTaskDashboardCharts`).
- `POST /api/tasks` — create, accepts `assigneeId` (fires `notifyTaskAssigned` unless self-assigned), `startsAt`, `dueAt`, `estimateHours`, `tagIds[]`, `kind` (validated; defaults `TASK`)
- `POST /api/tasks/bulk` — bulk create (writable member; max 500). Body `{ projectId, tasks[] }` with per-row `kind` ∈ `TASK|BUG|QC|TICKET|IDEA` (all five, matching single-create; defaults `TASK`). All-or-nothing: any invalid row → 400 `{ error: 'Validation failed', errors[] }`, nothing created. `assigneeEmail`/`tagNames`/`phaseName` are resolved against the project (unknown → 400). Helpers: `src/routes/tasks/bulk.helpers.ts` (`normalizeBulkRows`/`resolveBulkRefs`). Frontend CSV headers: `title,description,kind,priority,startsAt,dueAt,estimateHours,assigneeEmail,tagNames,phaseTitle` (`src/frontend/lib/csv.ts`).
...
- `POST /api/tasks/reorder` — Kanban drag-and-drop (writable member). Body `{ updates: [{ id, kanbanOrder, status? }] }`, applied to any number of tasks in one call (multi-select drag). `status` is only present when the drop moved the card to a different column — a same-column reorder omits it. When `status` **is** present and differs from the task's current status, the same side effects `PATCH /api/tasks/:id` applies also fire here: `closedAt` set on `→CLOSED`, cleared on `→REOPENED`, a `TaskStatusChange` row written, and `notifyTaskStatusChanged` fired. A pure `kanbanOrder`-only update touches none of these — was a real bug (task closed via Kanban drag got `status=CLOSED` but no `closedAt`, so it silently never appeared in the Throughput chart's "Closed" series or in `actualHours`/effort calculations). No server-side transition validation (`getAllowedTaskTransitions`) — the client's `kanbanAllowed()` gates which drops are offered; unlike `PATCH /api/tasks/:id` this endpoint currently trusts the client. Helpers: `src/routes/tasks/reorder.helpers.ts` (`buildReorderUpdateData`, `applyReorderStatusSideEffects`).
