...
- Schema: `prisma/schema.prisma`
  ...
  - `ProjectPhase` (id, projectId, `createdById?`, title, description?, summary?, status=PhaseStatus, order, startsAt?, endsAt?, timestamps) — fase/sprint per project; `summary` diisi saat menutup fase (ACTIVE→COMPLETED). Tasks link via nullable FK `phaseId` with `onDelete: SetNull`. `createdById` (nullable, FK→User `onDelete: SetNull`) menyimpan pembuat phase untuk aturan otorisasi PM-creator (di bawah). Enum `PhaseStatus` = `PLANNING | ACTIVE | COMPLETED`. **`title` unik per project (case-insensitive, trim)** — divalidasi di handler/MCP (bukan DB constraint, karena constraint DB case-sensitive), tolak `409` di create + rename. Sumber kebenaran: `isPhaseNameTaken()` di `src/lib/phase-name.ts`, dipakai HTTP handler (`phases.handlers.ts`) & MCP (`phase_create`/`phase_update`). Frontend (`PhaseAddForm`/`PhaseEditModal`) cek lokal → disable submit + hint sebelum request. **Otorisasi phase** (`src/lib/phase-access.ts`, TERPISAH dari `canManageProject` yang tak berubah): **create** = project OWNER/PM atau SUPER_ADMIN; **update/delete** = project OWNER, SUPER_ADMIN, atau PM. PM boleh modifikasi phase lama tanpa creator tercatat (`createdById=null` — diperlakukan sebagai milik project), tapi untuk phase yang punya creator tercatat, PM **hanya boleh modifikasi phase miliknya sendiri** (`createdById === userId`). Catatan: ADMIN biasa (non-SUPER_ADMIN) TIDAK dapat bypass sistem di phase — hanya lolos lewat membership project-nya (beda dari `isSystemAdmin`). Frontend mirror lewat `canCreatePhaseFE`/`canModifyPhaseFE` (`ProjectDetailView/types.ts`), gating menu aksi per-phase. MCP `phase_create` terima `actorEmail` opsional untuk isi `createdById`; MCP tetap admin-scope (tak menegakkan aturan PM-creator, tak ada sesi user).
...
- Google OAuth: `GET /api/auth/google` → Google → `GET /api/auth/callback/google` — upserts user, creates session. Redirect URI is built via `getPublicOrigin(request)` which honors `BETTER_AUTH_URL`, then `X-Forwarded-Proto`/`X-Forwarded-Host` (behind Traefik TLS termination), falling back to `request.url`. Prevents `redirect_uri_mismatch` when the app sits behind a reverse proxy. `socialProviders.google.prompt = 'select_account'` (`src/lib/auth.ts`) forces Google's account chooser on every sign-in — without it, Google silently reuses whichever Google account is already active in the browser (skipping the chooser) whenever exactly one Google session exists there, even across unrelated apps sharing that browser, which could auto-create/select the wrong pm-dashboard user.
