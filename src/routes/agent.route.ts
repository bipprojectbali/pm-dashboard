...
function deny(set: { status?: number | string }, status: number, error: string) {
  set.status = status
  return { error }
}
...
async function ownedTask(taskId: string, projectId: string) {
...
// token's project; agents never pass projectId. See llms.txt + docs/API.md.
  ...
  return new Elysia()
    .get('/api/agent/tasks', async ({ request, query, set }) => {
      ...
      if (query.assigneeEmail) {
        const u = await prisma.user.findUnique({ where: { email: String(query.assigneeEmail) }, select: { id: true } })
