...
### REST agent surface (CLI) — `/api/agent/*` + `llms-agent.txt`
...
- **`GET /llms-agent.txt`** (auth-gated): dokumentasi mesin-readable (format llmstxt.org) berisi semua endpoint + contoh `curl`, base URL dari request origin. **Bukan publik** — butuh `pmt_` token ATAU sesi login (anonim → 401); ini tool internal, dinamai `-agent` (bukan konvensi publik `llms.txt`) agar tak auto-discovered crawler. Ini defense-in-depth: batas keamanan asli tetap di validasi token pada `/api/agent/*`. Konten di `src/lib/llms-content.ts`; ditambahkan ke `isApiRoute` (`src/index.tsx`) agar tak kena SPA fallback. URL + snippet curl juga ditampilkan di show-once modal saat buat token.
...
- **Kapan pakai apa**: MCP untuk Claude Code interaktif (tool discovery, multi-step). REST/`llms-agent.txt` untuk CI, cron, one-shot command, atau agent non-MCP.
