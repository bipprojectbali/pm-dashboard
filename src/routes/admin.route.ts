...
import { extractSessionToken, getIp, isSystemAdmin, requireAuth } from '../lib/route-helpers'
...
export function adminRoutes() {
  return (
    new Elysia()
      ...
      // ─── File Health API (SUPER_ADMIN only) ──────────────
      .get('/api/admin/file-health', async ({ request, set }) => {
        ...
        function getFileMeta(rel: string): {
          skip: boolean
          type?: FileType
          limitLines?: number
          limitChars?: number
      ...
      .post('/api/admin/sync/pull', async ({ request, set }) => {
        ...
        const { entities: data } = (await exportRes.json()) as {
          exportedAt: string
        ...
        if (data.tasks && want('tasks')) {
          ...
          const taskTags = (data.tasks as TaskRow[]).flatMap((t) =>
            ((t.tags as Array<{ tagId: string }> | undefined) ?? []).map((tt) => ({
          ...
          if (checklists.length)
            await ins(prisma.taskChecklistItem.createMany.bind(prisma.taskChecklistItem), checklists)
          ...
          const statusChanges = (data.tasks as TaskRow[]).flatMap(
            (t) => (t.statusChanges as unknown[] | undefined) ?? [],
          )
          if (statusChanges.length)
            await ins(prisma.taskStatusChange.createMany.bind(prisma.taskStatusChange), statusChanges)
        ...
        appLog(
          'info',
          `Sync pull from ${remoteUrl} by userId=${session.user.id} — entities: ${entities.join(',') || 'all'}`,
          ip,
        )
