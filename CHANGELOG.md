...
- **Edit nama di Pengaturan**: tab Profil kini punya ikon pensil di samping nama untuk mengubahnya langsung. Endpoint baru `PUT /api/me/profile`, MCP tool baru `admin_update_user_profile`.
- **User tanpa proyek kini muncul di filter "Anggota" pada menu Proyek**: strip avatar dan dropdown "Anggota" sebelumnya hanya menampilkan user yang jadi owner/member di salah satu proyek yang terlihat — user yang belum tergabung ke proyek manapun (mis. akun baru) sama sekali tidak muncul, sehingga tak bisa dipakai untuk mengecek "kenapa user ini tidak ada di manapun". Kini user tanpa proyek tetap ditampilkan, ditaruh di akhir list dengan label "belum ada proyek", memakai endpoint `GET /api/users` yang sudah ada (tidak ada endpoint baru).

### Diperbaiki
- **Ekspor "Simpan PDF" di Laporan Lengkap gagal dengan pesan `[object Event]`**: penyebabnya foto profil Google pada laporan per-user tidak bisa diambil ulang oleh library screenshot (`html-to-image`) karena dibatasi CORS lintas-domain — kegagalan itu memicu event error mentah pada elemen gambar yang tidak tertangani, lalu ditampilkan apa adanya sebagai `[object Event]`. Kini gambar yang gagal diambil digantikan placeholder kosong sehingga PDF tetap berhasil dibuat.
...
### Diperbaiki
- **User yang di-block masih muncul di picker member proyek aktif**: user yang statusnya di-block di User Management sebelumnya masih tampil di avatar filter "Anggota" pada halaman Proyek serta muncul sebagai opsi Assignee saat membuat/mengubah task — padahal user tersebut sudah tidak bisa login. Penyebabnya block hanya menghapus sesi & set status, tidak pernah menyentuh data keanggotaan proyek (`ProjectMember`), sementara endpoint `GET /api/projects`/`GET /api/projects/:id` tidak pernah memfilter status blocked. Kini user yang di-block dikecualikan dari avatar/filter "Anggota" dan dropdown Assignee, tapi tetap tampil (dengan badge "Blocked") di halaman manajemen Team & Overview proyek agar admin masih bisa melihat dan mengeluarkannya — data keanggotaan sendiri tidak dihapus, jadi unblock langsung memulihkan visibilitas penuh.
