...
- **Helpers**: `src/lib/admin-overview.ts`
  ...
  - `computeAnalytics({ timelineLimit?, trendDays? })` — chart-ready aggregates: `projectsByStatus`, `tasksByStatus`, `timeline` (active projects with startsAt/endsAt/slipped flag), `deadlineGroups` (pastDue / endingSoon <7d / endingMonth 7–30d), `taskTrend` (created vs closed per day, last N days, default 14, max 90 — the endpoint cap matches this so the Analytics "90 hari" window is not silently shrunk). All task aggregates exclude `IDEA` via `WORKLOAD_KIND_FILTER`. The Analytics tab's stat cards read from this aggregate — **Proyek Aktif** ← `projectsByStatus.ACTIVE`, **Task Terbuka** ← Σ non-CLOSED `tasksByStatus`, **Ditutup (Nh)** ← Σ `taskTrend.closed` — so they stay accurate and IDEA-free regardless of the 200-row task cap. Only **Avg Cycle** and the per-task charts (Kontributor/Cycle Time/Aging WIP/WIP-per-Proyek) are computed client-side over the capped `/api/tasks?limit=200` list (IDEA filtered out in the client too); a banner appears when `total > 200`.
  - `computeTaskTriage({ projectId?, staleDays? })` — the 5 stat-card counts behind the Admin **Task Triage** tab: `total` (open = non-CLOSED), `overdue` (open + `dueAt < now`), `unassigned` (open + no assignee), `blocked` (open + ≥1 `blockedBy`), `stale` (open + `updatedAt` older than `staleDays`, default 7). Counts are computed **directly in the DB** (not client-side over a capped `/api/tasks` page) and exclude `IDEA` via `WORKLOAD_KIND_FILTER`. `overdue` uses the same "open + past due" definition as `computeRiskReport`, so the sidebar "task overdue" badge and this card always agree.
  - `computeUserReport({ userId, trendDays? })` — per-user drill-down snapshot (same shape family as the cockpit aggregates above, scoped to one `assigneeId`): `total`/`open`/`closed`/`overdue`/`blocked` counts, `byStatus`/`byPriority`/`byKind` breakdowns, `effort` (`actualHours`/`estimateHours` summed over closed tasks via `computeActualHours`, plus `over`/`under`/`on` counts at the same ±25% variance threshold used elsewhere), `github7d` (`ProjectGithubEvent` grouped by `kind` for `matchedUserId = userId`, last 7 days), `overdueTasks` (top 5 for drill-down), and `taskTrend` (created vs closed per day, same day-bucketing as `computeAnalytics`, default 14 days, max 90). Returns `null` if `userId` doesn't resolve to a user — the route translates that to 404. All task aggregates exclude `IDEA` via `WORKLOAD_KIND_FILTER`.
- **API** (ADMIN + SUPER_ADMIN):
  ...
  - `GET /api/admin/overview/triage?projectId&staleDays` — Task Triage stat-card counts (see `computeTaskTriage`). The triage table itself still lists via `GET /api/tasks` (capped 200, ordered OPEN-first so open work survives truncation), but the cards read this aggregate so they stay accurate past the cap.
  - `GET /api/admin/overview/analytics?timelineLimit&trendDays`
  - `GET /api/admin/overview/user-report?userId&trendDays` — per-user drill-down (see `computeUserReport`). `userId` required (400 if missing), 404 if the user doesn't exist. `trendDays` clamps 1–90, default 14 (same convention as `analytics`).
- **Frontend**: `OverviewPanel.tsx` — KPI cards (skeleton-loading) + Red flags (severity badge + 6 risk stats + overdue top-5 + past-due projects list) + Portfolio health grid (card per project with A-F badge, clickable) + Team load bars (Progress per user colored by overloaded-threshold) + **AnalyticsSection** (Gantt-style project timeline with today marker, status-breakdown donuts for projects+tasks, task-trend line chart with Created/Closed series, deadline-groups 3-column list) — all via ECharts (`src/frontend/components/charts/EChart.tsx` wrapper). Red flags refresh 30s; health + load + analytics refresh 60s. Empty states shown when no data. **Per-user drill-down**: a user picker in the panel header (or clicking a row in Team load) swaps the panel's content to `UserReportPanel.tsx`, which renders the same shape of KPIs/charts as the aggregate view but scoped to `GET /api/admin/overview/user-report?userId=`; a clear action returns to the aggregate view. No separate tab/route — it's a mode within the existing Ringkasan tab.

## Portfolio Report ("Laporan Lengkap")

Printable/PDF report page at `/admin/report`, reached via the "Laporan Lengkap" button in the Ringkasan tab. Two modes, mutually exclusive:

- **Portfolio mode** (default) — system-wide report over a period preset (Bulan ini / 30 hari / 90 hari / Tahun ini): cover + executive summary, project health grid, risk radar, project timeline (Gantt), status/priority distribution donuts, velocity trend, team load, GitHub activity, audit highlights.
  - **API**: `GET /api/admin/report?since&until` (ADMIN + SUPER_ADMIN) — aggregates `computeAdminOverview`/`computeProjectHealth`/`computeRiskReport`/`computeTeamLoad`/`computeAnalytics` (all from `src/lib/admin-overview.ts`) plus period-scoped raw queries (priority groups, GitHub commit/PR/review counts per project, closed/created task counts, audit log highlights). `trendDays` for the analytics/velocity section is derived from `since`/`until`, clamped 7–60. 400 on invalid/inverted `since`/`until`.
  - **Frontend**: `src/frontend/routes/admin_.report.tsx` + `admin_report/*` (CoverExecutive, ProjectSections, ChartSections, TeamEffortSections, ActivitySections, shared, constants, types). PDF export via `src/frontend/lib/report-pdf.ts` — screenshots every `.page-section` element into one PDF page each (`html-to-image` + `jspdf`), theme-agnostic (captures whatever color scheme is active).
- **Per-user mode** — a user picker (Select, searchable) in the page header swaps the *entire* report to be about that one person (not portfolio + an extra section): cover + executive summary scoped to the user, effort card, task breakdown donuts, task trend chart, overdue list (reusing the exact same components as the Ringkasan drill-down, see `src/frontend/components/admin/userreportpanel/*`), plus the same GitHub activity / audit highlights sections scoped to that user. Clearing the picker returns to the portfolio report.
  - **API**: `GET /api/admin/report/user?userId&since&until` (ADMIN + SUPER_ADMIN) — reuses `computeUserReport` (see Admin Overview Cockpit above) for the snapshot fields, plus period-scoped GitHub activity (`ProjectGithubEvent` filtered by `matchedUserId`) and audit highlights (`AuditLog` filtered by `userId` as actor) in the same shape as the portfolio endpoint's `github`/`audit` fields, so `GithubActivitySection`/`AuditHighlightsSection`/`FooterSection` are reused unmodified. `userId` required (400 if missing), 404 if unknown. Same `since`/`until` validation and `trendDays` derivation as the portfolio endpoint.
  - **Frontend**: `src/frontend/routes/admin_report/UserReportSections.tsx` (`UserCoverSection`, `UserExecutiveSummary`) + reused Ringkasan drill-down components (extended with optional `className`/`renderer` props so they can be tagged `page-section` for PDF capture and render charts as SVG for crisper export, matching the portfolio report's convention).
  - PDF filename becomes `user-report-<email-local-part>-<preset>-<date>.pdf` in this mode.
