...
export async function computeProjectHealth(
  ...
  const byProject = new Map<string, { status: Record<string, number>; overdue: number; closed7d: number; blocked: number }>()
  for (const p of projects) byProject.set(p.id, { status: {}, overdue: 0, closed7d: 0, blocked: 0 })
  for (const s of statusGroups) { const b = byProject.get(s.projectId); if (b) b.status[s.status] = s._count }
  for (const o of overdueGroups) { const b = byProject.get(o.projectId); if (b) b.overdue = o._count }
  for (const c of closedGroups) { const b = byProject.get(c.projectId); if (b) b.closed7d = c._count }
  for (const d of blockedRaw) { const b = byProject.get(d.task.projectId); if (b) b.blocked += 1 }

  const results: ProjectHealthRow[] = projects.map((p) => {
    const bucket = byProject.get(p.id)!
    const openTotal = Object.entries(bucket.status).filter(([k]) => k !== 'CLOSED').reduce((n, [, v]) => n + v, 0)
    ...
    return {
      ...
      endsAt: p.endsAt, daysUntilDue, pastDue, counts: p._count, taskStatus: bucket.status,
      openTasks: openTotal, overdueTasks: bucket.overdue, blockedTasks: bucket.blocked,
      closed7d: bucket.closed7d, extensions, score, grade,
    }
  })
