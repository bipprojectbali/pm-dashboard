...
- `/api/agent/*` (incl. `GET /api/agent/guide`) — token-only REST surface (lighter than MCP) for CLI/scripts. `src/routes/agent.route.ts` (barrel → `src/routes/agent/{guide,tasks,checklist}.route.ts` + `shared.ts`) + `src/lib/agent-auth.ts`; **auth-gated** guide at `/api/agent/guide` (`src/lib/llms-content.ts`) — requires a `pmt_` token OR a logged-in session (not public; internal tool). Isolated from session routes.
