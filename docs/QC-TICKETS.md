...
- **QC API** (QC + ADMIN + SUPER_ADMIN; bypasses project membership — QC role implicitly grants access to the self-project only):
  ...
  - `GET /api/qc/tickets?status=&priority=&q=&sort=&order=` — list tickets in self-project tagged `ai-queue`. `q` = case-insensitive substring over title/description/route. `sort` ∈ `priority|created|updated|title`, `order` ∈ `asc|desc` (default sort: `priority` desc then `createdAt` desc).
