...
- **Frontend**: `/qc` route (`src/frontend/routes/qc.tsx`) — AppShell with stats card sidebar, SegmentedControl for status filter (open/in-progress/ready/closed/all), debounced search box (300ms; matches title/description/route), sortable column headers (Title/Priority/Tanggal — click to toggle asc/desc), ticket table with pagination (25/page; page state in `?page=`, resets to 1 on filter/search/sort change), create modal (title/description/priority/route/evidence URLs newline-separated), and Drawer detail via `?ticketId=` search param. Filter/search/sort state persists in URL search params (`?status=&q=&sort=&order=`). Drawer supports inline edit mode (title/description/route via Edit → Simpan/Batal) and Delete button (ADMIN/SUPER_ADMIN only, confirm modal). Status + priority Select always-editable, evidence add form, comments thread + add, Timeline of status changes. `beforeLoad` gates: unauth → `/login`, blocked → `/blocked`, non-QC/ADMIN/SUPER_ADMIN → `/pm`.
