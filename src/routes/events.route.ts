    ...
    new Elysia()
      ...
      .patch('/api/event-tags/:id', async ({ request, params, set }) => {
        ...
        if (!isSystemAdmin(auth.role)) {
          set.status = 403
          return { error: 'Hanya admin yang bisa mengedit tag' }
        }
        const body = (await request.json()) as { name?: string; color?: string }
      ...
      .delete('/api/event-tags/:id', async ({ request, params, set }) => {
        ...
        if (!isSystemAdmin(auth.role)) {
          set.status = 403
          return { error: 'Hanya admin yang bisa menghapus tag' }
        }
      ...
      .post('/api/events', async ({ request, set }) => {
        ...
        if (!body.startsAt) {
          set.status = 400
          return { error: 'startsAt wajib diisi' }
        }
        const startsAt = new Date(body.startsAt)
        if (Number.isNaN(startsAt.getTime())) {
          set.status = 400
          return { error: 'startsAt tidak valid' }
        }
        const endsAt = body.endsAt ? new Date(body.endsAt) : undefined
        if (endsAt && Number.isNaN(endsAt.getTime())) {
          set.status = 400
          return { error: 'endsAt tidak valid' }
        }
      ...
      .patch('/api/events/:id', async ({ request, params, set }) => {
        ...
        if (!isAdmin && !isCreator) {
          set.status = 403
          return { error: 'Hanya kreator atau admin yang bisa mengedit' }
        }
      ...
      .delete('/api/events/:id', async ({ request, params, set }) => {
        ...
        if (!isAdmin && !isCreator) {
          set.status = 403
          return { error: 'Hanya kreator atau admin yang bisa menghapus' }
        }
