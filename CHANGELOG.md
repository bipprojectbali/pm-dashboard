...
- **Edit nama & foto profil di Pengaturan**: tab Profil kini punya ikon pensil di samping nama untuk mengubahnya langsung, dan ikon kamera di avatar untuk unggah foto profil baru (atau menghapusnya kembali ke inisial). Endpoint baru `PUT /api/me/profile`, `POST/GET/DELETE /api/me/avatar` (foto disimpan di MinIO yang sama dengan evidence task), MCP tool baru `admin_update_user_profile` (khusus ubah nama — unggah foto tidak diekspos ke MCP karena butuh multipart biner).
