...
- `pm.tsx` — Project management shell (all authenticated users) — overview, projects, tasks, tickets, ideas, team, events tabs. **Tickets** (`KindBoardPanel kind=TICKET`) and **Pengembangan/ideas** (`KindBoardPanel kind=IDEA`) are cross-project boards reusing the admin triage components, scoped to one `kind`. Both are **server-side**: stat cards come from `GET /api/tasks/kind-board-stats` (accurate, un-capped, visibility-scoped) and the table paginates via `GET /api/tasks?kind=…&limit&offset` with every filter (status/priority/assignee/search/overdue/blocked/`staleDays`) sent to the server — no more client-side filtering over a 200-cap page. The Create modal is shared `CreateTaskModal` with a `heading` prop ("Create Ticket" / "Create Idea"; menu Task stays "Create Task"). Tickets is the daily-standup triage board; ideas is the "biar tidak lupa" backlog, promoted to a Task via the detail sidebar's "Naik Kelas → Jadikan Task" button.
