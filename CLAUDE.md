...
- **deploy MCP** (`scripts/mcp-deploy/server.ts`) — Bun stdio server wrapping `gh` CLI. Tools: `publish_docker`, `re_pull`, `run_status`, `run_wait`, `run_logs`, `run_list`, `bump_version`, and `deploy_stg` (publish → wait → re-pull → wait → tag `stg-v<version>`). `GH_DEPLOY_REPO` env sets target repo (default `bipprojectbali/pm-dashboard`).
